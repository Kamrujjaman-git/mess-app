...
@section('title', 'Add maid bill — Mess App')

@section('content')
    <div class="mx-auto max-w-lg">
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">Add maid bill</h1>
        <p class="mt-1 text-sm text-slate-500">Record a maid bill for a member. Month uses <code class="rounded bg-slate-100 px-1.5 py-0.5 text-xs font-medium text-slate-700">Y-m</code> format.</p>

        @if ($users->isEmpty())
            <div class="form-callout mt-6">
                Add at least one user before recording maid bills.
        ...
        <form action="{{ route('maid-bills.store') }}" method="post" class="form-card" novalidate>
            @csrf

            <div>
                <label for="maid-bill-user-id" class="form-label">User</label>
                <select name="user_id" id="maid-bill-user-id" required @disabled($users->isEmpty())
            ...
            <div>
                ...
                <input type="number" name="amount" id="maid-bill-amount" value="{{ old('amount') }}"
            ...
            <div class="form-actions">
                ...
                <a href="{{ route('maid-bills.index', ['month' => old('month', request('month', date('Y-m')))]) }}"
